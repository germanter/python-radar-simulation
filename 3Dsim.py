...
class S400_Russian_Missile:
    def __init__(self):
        self.area = plt.figure()
        self.ax = self.area.add_subplot(111, projection='3d')
        self.ax.set_xlim([-max_spawn_dist, max_spawn_dist])
        self.ax.set_ylim([-max_spawn_dist, max_spawn_dist])
        self.ax.set_zlim([0, 35000])
        self.ax.set_xlabel('X (meters)')
        self.ax.set_ylabel('Y (meters)')
        self.ax.set_zlabel('Z (meters)')
        self.ax.set_title('S400 Simulation')

        self.show_plane, = self.ax.plot([], [], [], 'bo', label='Plane')
        self.show_plane_track, = self.ax.plot([], [], [], 'b-', alpha=0.5, label='Plane Track')
        self.show_missile, = self.ax.plot([], [], [], 'ro', label='Missile')
        self.show_missile_track, = self.ax.plot([], [], [], 'r-', alpha=0.5, label='Missile Track')
        self.show_plane_missile, = self.ax.plot([], [], [], 'go', label='Plane Missile')
        self.show_plane_missile_track, = self.ax.plot([], [], [], 'g-', alpha=0.5, label='Plane Missile Track')
        self.show_defense_missile, = self.ax.plot([], [], [], 'yo', label='Defense Missile')
        self.show_defense_missile_track, = self.ax.plot([], [], [], 'y-', alpha=0.5, label='Defense Missile Track')
        self.ax.legend()

        self.table_text = self.ax.annotate('', xy=(0.00, 0.75), xycoords='figure fraction',
                                          fontsize=14, fontweight='bold', color='#0DF517',
                                          bbox=dict(facecolor='black', edgecolor='gray', alpha=1),
                                          verticalalignment='top')

        self.fire_button = Button(plt.axes([0.85, 0.05, 0.1, 0.075]), 'FIRE MISSILE', color='#C80000', hovercolor='#300000')
        self.fire_button.label.set_fontproperties(fm.FontProperties(family='Impact', size=12, weight='bold'))
        self.fire_button.label.set_color('white')
        self.fire_button.ax.set_facecolor('#8B0000')
        for spine in self.fire_button.ax.spines.values():
            spine.set_color('black')
            spine.set_linewidth(2)

        self.destruct_button = Button(plt.axes([0.70, 0.05, 0.1, 0.075]), 'SELF DESTRUCT', color='#DB6A00', hovercolor='#934E00')
        self.destruct_button.label.set_fontproperties(fm.FontProperties(family='Impact', size=12, weight='bold'))
        self.destruct_button.label.set_color('white')
        self.destruct_button.ax.set_facecolor('#4B0082')
        for spine in self.destruct_button.ax.spines.values():
            spine.set_color('black')
            spine.set_linewidth(2)
    ...
    def movement(self, current_ctd, target_ctd, h_max_rate, v_max_rate, xy_move, z_move, timer, it_is_missile=False, is_plane_missile=False):
        ...
        altitude_loss = direction[2] < 0  # if it is true gives True if not-> False this is new unnecessary thing that adnsu would like to see
        straight_move = abs(z_move) < 0.01  ## also same as the first # note giver

        ### explanation for why not save current speeds
        ### cus our speed is depends on V = V0 + (acceleration)*(time) so we dont need to save
    ...
    def update(self, frame):
        ...
        global plane_trail, missile_trail, plane_missile_trail, defense_missile_trail, plane_base_speed

        self.plane_ctd = self.plane_ctd  #giving it to global again
        ...
        if not plane_shot_down:
            if s400_destroyed:
                ...
                plane_trail.append(self.plane_ctd.copy())
            else:
        ...
        if not plane_shot_down and not missile_fired: ### doing this to avoid plane make circle around target when stuck
            if target_change_timer >= 50.0:
                print(f"Time is up change target: {self.target_ctds[self.current_target_idx]} -> {self.target_ctds[(self.current_target_idx + 1) % len(self.target_ctds)]}")
                self.current_target_idx = (self.current_target_idx + 1) % len(self.target_ctds)  ### just makes sure index is in range
                target_change_timer = 0.0
        ...
        if missile_fired:
            if missile_fuel > 0 and not no_fuel_mode and not buttondestruct and not s400_destroyed:
                ...
                missile_trail.append(missile_ctd.copy())
                self.fuel_consumption_rate(mspeed)
                if self.direction_dist_calc(missile_ctd, self.plane_ctd) < (missile_base_speed * timer): ### make it harder to catch
                    print(f'Target shot down by S400 missile at {missile_ctd}')
                    ...
                    print('Missile self destruction mode on')
                    no_fuel_mode = True
                    no_fuel_timer = 0.0
        ...
        if (used_missiles >= 1 and not missile_fired and not plane_shot_down and plane_type
                and not plane_missile_fired and not s400_destroyed and random.random() < 0.05):
            if (choose in {1, 3} and russian == -1) or choose not in {1, 3, 4}:
                print("Military plane retaliates with a missile!")
        ...
        if plane_missile_fired and not s400_destroyed:
            if plane_missile_fuel > 0:
                ...
                plane_missile_trail.append(plane_missile_ctd.copy())
                ...
                if self.direction_dist_calc(plane_missile_ctd, [0, 0, 0]) < (plane_missile_base_speed * timer):
                    print(f"S-400 destroyed by plane missile at {plane_missile_ctd}!")
                    s400_destroyed = True
                    plane_missile_fired = False
                    plane_missile_trail.clear()
                elif plane_missile_fuel <= 0:
                    print("Plane missile ran out of fuel and fell!")
                    plane_missile_fired = False
                    plane_missile_trail.clear()
        ...
        if (plane_missile_fired and not defense_missile_fired and not s400_destroyed and used_defense_missiles < max_defense_missiles):
            ...
            print(f"Plane missile distance to S-400: {distance_to_s400}")
            if distance_to_s400 <= 200000:
                print("S-400 launches defense missile!")
        ...
        if defense_missile_fired and not s400_destroyed:
            if defense_missile_fuel > 0:
                ...
                defense_missile_trail.append(defense_missile_ctd.copy())
                ...
                if self.direction_dist_calc(defense_missile_ctd, plane_missile_ctd) < (defense_missile_base_speed * timer):
                    print(f"Plane missile intercepted by defense missile at {defense_missile_ctd}!")
                    plane_missile_fired = False
                    defense_missile_fired = False
                    plane_missile_trail.clear()
                    defense_missile_trail.clear()
                elif self.direction_dist_calc(defense_missile_ctd, [0, 0, 0]) > 200000:
                    print(f"Defense missile self-destructed at {defense_missile_ctd} due to range limit!")
                    defense_missile_fired = False
                    defense_missile_trail.clear()
            elif defense_missile_fuel <= 0:
                print("Defense missile ran out of fuel and fell!")
                defense_missile_fired = False
                defense_missile_trail.clear()

        if plane_missile_fired:
            self.show_plane_missile.set_data([plane_missile_ctd[0]], [plane_missile_ctd[1]])
            self.show_plane_missile.set_3d_properties([plane_missile_ctd[2]])
            self.show_plane_missile_track.set_data([m[0] for m in plane_missile_trail], [m[1] for m in plane_missile_trail])
            self.show_plane_missile_track.set_3d_properties([m[2] for m in plane_missile_trail])
            if len(plane_missile_trail) > 100:
                plane_missile_trail.pop(0)
        else:
            self.show_plane_missile.set_data([], [])
            self.show_plane_missile.set_3d_properties([])
            self.show_plane_missile_track.set_data([], [])
            self.show_plane_missile_track.set_3d_properties([])

        if not plane_shot_down:
            self.show_plane.set_data([self.plane_ctd[0]], [self.plane_ctd[1]])
            self.show_plane.set_3d_properties([self.plane_ctd[2]])
            self.show_plane_track.set_data([p[0] for p in plane_trail], [p[1] for p in plane_trail])
            self.show_plane_track.set_3d_properties([p[2] for p in plane_trail])
            if len(plane_trail) > 100:
                plane_trail.pop(0)
        else:
            self.show_plane.set_data([], [])
            self.show_plane.set_3d_properties([])
            self.show_plane_track.set_data([], [])
            self.show_plane_track.set_3d_properties([])

        if missile_fired:
            self.show_missile.set_data([missile_ctd[0]], [missile_ctd[1]])
            self.show_missile.set_3d_properties([missile_ctd[2]])
            self.show_missile_track.set_data([m[0] for m in missile_trail], [m[1] for m in missile_trail])
            self.show_missile_track.set_3d_properties([m[2] for m in missile_trail])
            if len(missile_trail) > 100:
                missile_trail.pop(0)
        else:
            self.show_missile.set_data([], [])
            self.show_missile.set_3d_properties([])
            self.show_missile_track.set_data([], [])
            self.show_missile_track.set_3d_properties([])

        if defense_missile_fired:
            self.show_defense_missile.set_data([defense_missile_ctd[0]], [defense_missile_ctd[1]])
            self.show_defense_missile.set_3d_properties([defense_missile_ctd[2]])
            self.show_defense_missile_track.set_data([m[0] for m in defense_missile_trail], [m[1] for m in defense_missile_trail])
            self.show_defense_missile_track.set_3d_properties([m[2] for m in defense_missile_trail])
            if len(defense_missile_trail) > 100:
                defense_missile_trail.pop(0)
        else:
            self.show_defense_missile.set_data([], [])
            self.show_defense_missile.set_3d_properties([])
            self.show_defense_missile_track.set_data([], [])
            self.show_defense_missile_track.set_3d_properties([])
        ...
        table_content = (
            ...
            f"----------- Spectator mode -----------\n"
            f"Plane missile fuel : {plane_missile_fuel}\n"
            f"{keys[choose]}\n"
            f"{russian}"
        )
        self.table_text.set_text(table_content)

        if self.direction_dist_calc(self.plane_ctd, target_ctd) < plane_base_speed * timer:
            print(f'Reached target {self.target_ctds[self.current_target_idx]}')
            self.current_target_idx += 1
            if self.current_target_idx >= len(self.target_ctds):
                self.current_target_idx = 0
            radio = 1
            target_change_timer = 0.0
        ...
        t += timer
        print(target_ctd)
        return (self.show_plane, self.show_plane_track, self.show_missile, self.show_missile_track,
                self.show_plane_missile, self.show_plane_missile_track, self.show_defense_missile, self.show_defense_missile_track, self.table_text)

    def main(self):
        ani = FuncAnimation(self.area, self.update, frames=None, interval=50, blit=False)
        plt.show()
